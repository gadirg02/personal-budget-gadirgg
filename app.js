...
let cloudConfig = JSON.parse(localStorage.getItem(CLOUD_CONFIG_KEY) || 'null') || { url: '', key: '', enabled: false };
...
function normalizeCloudConfig(){
  cloudConfig.url = (cloudConfig.url || '').trim();
  cloudConfig.key = (cloudConfig.key || '').trim();
...
async function initCloud(){
  ...
  if(cloudUser){ cloudStatus='Облако подключено'; await cloudLoad(false); }
  else cloudStatus='Supabase настроен, нужен вход';
  cloudClient.auth.onAuthStateChange((_event, session)=>{ cloudUser = session?.user || null; cloudStatus = cloudUser ? 'Облако подключено' : 'Supabase настроен, нужен вход'; render(); });
}
function scheduleCloudSave(){
  if(suppressCloudSave || !cloudClient || !cloudUser) return;
  clearTimeout(cloudSaveTimer);
  cloudSaveTimer = setTimeout(()=>cloudSave(false), 900);
}
async function cloudSave(showAlert=true){
  ...
  if(error){ cloudStatus='Ошибка сохранения в облако'; console.error(error); if(showAlert) alert('Ошибка сохранения: '+error.message); render(); return; }
  cloudStatus='Сохранено в облаке';
  if(showAlert) alert('Сохранено в Supabase');
  render();
}
...
function cloudPanel(){
  const maskedKey = cloudConfig.key ? cloudConfig.key : '';
  return `<div class="card" style="margin-top:14px"><h3>Облако Supabase</h3><p>Нужно, чтобы данные были доступны с телефона, рабочего ПК и любого браузера. Локальное сохранение остается как резерв.</p>
  <div class="cloudStatus"><span class="pill">${escapeHtml(cloudStatus)}</span>${cloudUser?`<span class="pill">${escapeHtml(cloudUser.email || '')}</span>`:''}</div>
  <div class="formrow settingsForm"><label><input type="checkbox" id="supabaseEnabled" ${cloudConfig.enabled?'checked':''}> Включить облачный режим</label><input id="supabaseUrl" placeholder="Supabase Project URL" value="${escapeHtml(cloudConfig.url)}"><input id="supabaseKey" placeholder="Publishable / anon key" value="${escapeHtml(maskedKey)}"><button onclick="saveCloudSettings()">Сохранить настройки</button></div>
  <div class="formrow settingsForm"><input id="cloudEmail" type="email" placeholder="Email"><input id="cloudPassword" type="password" placeholder="Пароль"><button onclick="cloudSignUp()">Регистрация</button><button onclick="cloudLogin()">Войти</button><button onclick="cloudLoad(true)">Загрузить из облака</button><button onclick="cloudSave(true)">Сохранить в облако</button><button class="danger" onclick="cloudLogout()">Выйти</button></div>
  <p class="mutedText">В Supabase обязательно включи RLS и создай политики из файла <strong>supabase-schema.sql</strong>. Secret/service_role key сюда вставлять нельзя.</p></div>`;
}
